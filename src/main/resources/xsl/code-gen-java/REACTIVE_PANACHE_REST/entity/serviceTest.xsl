...
public class <xsl:value-of select="entity/@name" />ServiceTest {
    ...
    public void findRecordByIdShouldBeCalled() {
        when(repository.findRecordById(anyLong())).thenReturn(Uni.createFrom().nullItem());
        service.findById(1L);
        verify(repository).findRecordById(anyLong());
    ...
    public void saveRecordShouldBeCalled() {
        ...
        when(repository.save(any())).thenReturn(Uni.createFrom().nullItem());
        service.save(record);
