...
public class <xsl:value-of select="entity/@name" />ResourceIT {
    ...
    public void beforeEach() {
        record = <xsl:value-of select="entity/@name" />TestUtil.createRandomDto(null);
    }
    ...
    public void shouldFindRecordById() {
        ...
        given()
            ...
            .body("<xsl:value-of select="ggq:firstToLowerCase(concat(@name, 'Id'))" />", is(record.get<xsl:value-of select="ggq:firstToUpperCase(concat(@name, 'Id'))" />() != null ? record.get<xsl:value-of select="ggq:firstToUpperCase(concat(@name, 'Id'))" />().intValue() : null))
            </xsl:for-each>;
    }
