...
public class <xsl:value-of select="entity/@name" />Mapper {
    ...
    public static <xsl:value-of select="entity/@name" />Dto toDto(<xsl:value-of select="entity/@name" /> entity) {
        return new <xsl:value-of select="entity/@name" />Dto()
            ...
            </xsl:for-each><xsl:for-each select="entity/relations/relation">.<xsl:value-of select="@name" />Id(entity.get<xsl:value-of select="ggq:firstToUpperCase(@name)" />() != null ? entity.get<xsl:value-of select="ggq:firstToUpperCase(@name)" />().getId() : null)
            </xsl:for-each>;
    }
